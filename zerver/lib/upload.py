...
class S3UploadBackend(ZulipUploadBackend):
    ...
    def delete_file_from_s3(self, path_id: str, bucket: Bucket) -> bool:
        ...
        try:
            ...
            return False
        key.delete()
        logger.info(f"[zerver/upload.py] Deleted path='{path_id}' from S3")
        return True
    ...
    def upload_message_file(
        ...
        logger.info(
            "[zerver/lib/upload.py] Uploading file='%s', content-type='%s', user='%s', email='%s', target realm='%s'"
            % (uploaded_file_name, content_type, user_profile.full_name, user_profile.email, str(target_realm))
        )
        return url
...
def delete_local_file(type: str, path: str) -> bool:
    ...
    if os.path.isfile(file_path):
        ...
        os.remove(file_path)
        logger.info(f"[zerver/lib/upload.py] Deleted file from path={file_path}")
        return True
