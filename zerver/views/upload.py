...
def serve_file(
    ...
    if not is_authorized:
        logging.info(f"[views/upload.py][local] user='{maybe_user_profile.full_name}', is not authorized to access file='{filename}', path_id='{path_id}'")
        return HttpResponseForbidden(_("<p>You are not authorized to view this file.</p>"))
    if settings.LOCAL_UPLOADS_DIR is not None:
        logging.info(f"[views/upload.py][local] user='{maybe_user_profile.full_name}', is served with, file='{filename}', path_id='{path_id}'")
        return serve_local(request, path_id, url_only, download=download)
    logging.info(f"[views/upload.py][S3] user='{maybe_user_profile.full_name}', is served with, file='{filename}', path_id='{path_id}'")
